...
public:
    StatsController(httplib::Server &server, TSharkManager &tsharkManager)
    ...
    void getIPStatsList(const httplib::Request &req, httplib::Response &res) {
        ...
        try {
            ...
            std::vector<IPStatsInfo> ipStatsList;
            int total = 0;
    ...
    void getCountryStatsList(const httplib::Request& req, httplib::Response& res) {
        ...
        try {
            ...
            // 调用 tSharkManager 的方法获取数据
            std::vector<CountryStatsInfo> countryStatsList;
            int total = 0;
