...
public:
    SessionController(httplib::Server &server, TSharkManager &tsharkManager)
    ...
    void getSessionList(const httplib::Request &req, httplib::Response &res) {
        ...
        try {
            ...
            int total = 0;
            __tsharkManager.getSessionList(queryCondition, sessionList, total);
    ...
    void getSessionDataStream(const httplib::Request& req, httplib::Response& res) {
        ...
        try {
            ...
            // 提取参数字段
            if (doc.HasMember("sessionId") && doc["sessionId"].IsNumber()) {
                sessionId = doc["sessionId"].GetInt();
            }
