...
public:
    AdaptorController(httplib::Server &server, TSharkManager &tsharkManager)
    ...
    void getAdaptersFlowTrendData(const httplib::Request& req, httplib::Response& res) {
        ...
        try {
            ...
            // 做一下排序，把有数据的网卡放到前面
            std::vector<std::pair<std::string, std::map<long, long>>> flowTrendDataList1;
            std::vector<std::pair<std::string, std::map<long, long>>> flowTrendDataList2;
            for (const auto &adaptorItem : flowTrendData) {
                for (const auto &timeItem : adaptorItem.second) {
                    if (timeItem.second != 0) {
                        flowTrendDataList1.push_back(adaptorItem);
                        break;
                    }
                }
                flowTrendDataList2.push_back(adaptorItem);
            }
            flowTrendDataList1.insert(flowTrendDataList1.end(), flowTrendDataList2.begin(), flowTrendDataList2.end());
            ...
            for (const auto &adaptorItem : flowTrendDataList1) {
