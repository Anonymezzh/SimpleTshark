...
#include <chrono>
#include <codecvt>
#include <third_library/rapidxml/rapidxml.hpp>
...
public:
    ...
    static std::string getPcapNameByCurrentTimestamp(bool isFullPath=true) {
        ...
        // 格式化文件名
        char buffer[64];
        std::strftime(buffer, sizeof(buffer), "simpletshark_%Y-%m-%d_%H-%M-%S.pcap", localTime);
    ...
    static std::string getLogNameByCurrentTimestamp() {
        ...
        // 格式化文件名
        char buffer[64];
        std::strftime(buffer, sizeof(buffer), "simpletshark_%Y-%m-%d_%H-%M-%S.log", localTime);
...
#ifdef _WIN32
        dir = std::string(std::getenv("APPDATA")) + "\\simpletshark\\";
        dir = MiscUtil::ANSIToUTF8String(dir);      // Windows平台获取的环境变量默认是ANSI编码的，如果遇到中文会出问题，需要转换一下
#else
        dir = std::string(std::getenv("HOME")) + "/simpletshark/";
#endif
...
#ifdef _WIN32
        // Windows 系统使用 CopyFile 函数, 要进行编码转换，否则参数有中文会出错
        source = MiscUtil::UTF8ToANSIString(source);
        destination = MiscUtil::UTF8ToANSIString(destination);
    ...
    // 获取文件大小
    static std::uint64_t getFileSize(std::string filePath) {
        // 去除文件名前后的引号
        if (filePath[0] == '\"')
        ...
        if (STAT_FUNC(filePath.c_str(), &fileStat) == 0) {
            ...
            LOG_F(ERROR, "Could not get file size for: %s, errno=%d", filePath.c_str(), errno);
            return -1;
        }
    }
    ...
    // ANSI转UTF-8
    static std::string ANSIToUTF8String(const std::string str) {
        ...
        if (MultiByteToWideChar(CP_ACP, 0, str.c_str(), -1, &wstr[0], wlen) == 0) {
    ...
    static std::string UnicoeToANSIString(const std::wstring& wstr)
    {
    ...
    // 转换 IPv4 地址
    static std::string getIPv4(uint32_t ip) {
        in_addr addr;
        addr.S_un.S_addr = ip;
    ...
    static std::string getIPv6(const IN6_ADDR& ipv6addr) {
        char buffer[INET6_ADDRSTRLEN] = { 0 };
        const char* result = InetNtopA(AF_INET6, (PVOID)&ipv6addr, buffer, sizeof(buffer));
        return buffer;
    }
